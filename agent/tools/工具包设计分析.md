...
| 文件 | 类型 | 主要触发点 | 触发者 | 是否直接暴露给模型 |
|---|---|---|---|---|
| `__init__.py` | 工具聚合入口 | 导入 `agent.tools` 时 | Agent 工厂或其他模块 | 间接 |
| `github_tools.py` | 模型工具 | 模型选择 GitHub 工具时 | LangChain/DeepAgents | 是 |
| `reviewer_tools.py` | 模型工具 | 模型记录或读取审查发现时 | LangChain/DeepAgents | 是 |
| `fetch_url_tools.py` | 模型工具 | 模型决定读取指定 URL 时 | LangChain/DeepAgents | 是 |
| `web_search.py` | 模型工具 | 模型决定联网搜索时 | LangChain/DeepAgents | 是 |
| `github_api.py` | 内部业务服务 | GitHub 上层工具调用时 | `github_tools.py` 或中间件 | 否 |
| `safe_http.py` | 网络安全基础设施 | 网页请求及每次重定向前 | `fetch_url_tools.py` | 否 |
| `runtime_context.py` | 运行上下文基础设施 | 工具内部读取运行信息时 | 多个工具模块 | 否 |
...
`add_review_finding` 和 `list_review_findings` 都是 `@tool`：

- 模型发现一个具体问题时调用 `add_review_finding`；
- 模型准备汇总审查结果时调用 `list_review_findings`。
...
- `create_agent` 或 `create_deep_agent` 工厂；
- 将 `agent.tools` 中工具列表传入 Agent 的代码；
- 其他模块执行 `from agent.tools import ...` 的注册入口。

因此当前状态是：

> 工具已经通过 `@tool` 定义并在 `__init__.py` 导出，但没有看到它们被注册到实际 Agent。没有完成注册时，模型不会自动获得或触发这些工具。
...
`agent/store/__init__.py` 当前为空，但：

- `agent/core/events.py` 导入 `LocalSqliteStore`；
- GitHub 工具调用 `get_store().update_thread_status()`；
- Review 工具调用 `get_store().add_finding()` 和 `list_findings()`。

需要确认运行时注入的 Store 是否实现这些自定义方法，否则事件、任务状态和 Review 工具会失败。

### 16.2 GitHub 写权限检查不一致

`open_github_pull_request` 会检查 `runtime_is_read_only_task()`，但 `publish_github_pr_comment` 没有相同检查。评论同样是远程写操作，当前权限边界不一致。

### 16.3 缺少任务类型时默认允许写

`get_runtime_task_kind()` 在配置缺失或非法时回退为 `coding`。这有利于兼容旧调用链，但意味着缺少配置时会按可写任务处理，安全上属于偏宽松默认值。
