...
flowchart TB
    A["任务开始"] --> B["ContextInjectionMiddleware<br/>before_agent：注入仓库上下文"]
    B --> C["模型推理"]
    ...
    G -->|工具结果返回模型| C
    C -->|生成最终答复| H["任务结束"]
    H --> I["MemoryUpdateMiddleware<br/>after_agent：更新长期记忆"]
```
...
| `memory_update.py` | server 注册，DeepAgents 调用 | Agent 产生最终状态后的 `after_agent` | 每轮 Agent 一次 | 兜底写回仓库长期记忆 |
...
## 8. MemoryUpdateMiddleware兜底写回仓库长期记忆
...
- `agent/server.py:get_agent()` 把消息清洗、上下文注入、工具参数清洗、工具异常处理和记忆写回中间件传给 `create_deep_agent()`。
...
- runtime 成功路径和 middleware 兜底路径都可能尝试更新记忆，更新函数必须保持幂等和去重。
